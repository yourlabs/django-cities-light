...
class TestCitiesLigthFixtures(test.TransactionTestCase):
    ...
    def test_dump_fixture(self):
        ...
        try:
            fixture_path = os.path.join(
                os.path.dirname(__file__), "fixtures", "test_dump_fixture.json"
            )
            cmd = Command()
    ...
    def test_load_fixtures(self):
        ...
        with mock.patch.object(Command, "load_fixture") as mock_func:
            ...
            mock_func.assert_any_call(cmd.country_url, cmd.country_path, force=True)
            mock_func.assert_any_call(cmd.region_url, cmd.region_path, force=True)
            mock_func.assert_any_call(cmd.subregion_url, cmd.subregion_path, force=True)
            mock_func.assert_any_call(cmd.city_url, cmd.city_path, force=True)
    ...
    def test_incorrect_subcommand(self, m_exists):
        ...
        self.assertEqual(
            str(error.exception), 'Please specify either "load" or "dump" command'
        )
    ...
    def test_call_with_load_argument(self, m_exists):
        ...
        m_exists.return_value = True
        module = (
            "cities_light.management.commands."
