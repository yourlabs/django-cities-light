...
def subregion_fixture(country_fixture, region_fixture):
    return SubRegion.objects.create(
        name="Upper Normandy", country=country_fixture, region=region_fixture
    )
...
def test_subregion_type(db, country_fixture, subregion_fixture):
    Person.objects.create(
        name="Skippy", country=country_fixture, subregion=subregion_fixture
    )
    client = Client(schema)
    executed = client.execute(
        """{ people { name, subregion {name, region{name},  country{ name}} } }"""
    )
    returned_person = executed["data"]["people"][0]
    assert returned_person == {
        "name": "Skippy",
...
def test_city_type(db, country_fixture, city_fixture):
    ...
    client = Client(schema)
    executed = client.execute(
        """{ people { name, city{name, subregion {name, region{name},  country{ name}} } }}"""
    )
    returned_person = executed["data"]["people"][0]
    assert returned_person == {
        "name": "Skippy",
