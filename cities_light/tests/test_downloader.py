...
class TestDownloader(test.TransactionTestCase):
    ...
    def test_source_matches_destination(self, mock_func):
        ...
        #Exception handling, checking whether file exist or not, if exist then checking source and destination
        source = 'file:///data/a.txt'
        dest = '/data/a.txt'
    ...
    def test_download_calls_source_matches_destination(self, m_check):
        ...
        source = 'file:///a.txt'
        destination = '/a.txt'
        #The downloader.download will return false as source and destination are same
        #The downloader.source_matches_destination will return true and downloader.download will return false
        self.assertFalse(
            downloader.download(
                source,
                destination,
    ...
    def test_download_calls_needs_downloading(self, m_check, m_need):
        ...
        source = 'file:///a.txt'
        destination = '/a.txt'
        #Here dowaloder.needs_downloading() will return flase as the time of modifiaction of dest>= time of source
        # and the size od source and destination are same and downloader.download will return false
        self.assertFalse(
            downloader.download(
                source,
                destination,
    ...
    def test_download(self, m_check, m_need):
        ...
        with mock.patch('cities_light.downloader.urlopen',
            ...
            # The downoader.needs_downloading will return true and the last three lines of 
            # downloader.download will copy the source to sestination
    ...
    def test_not_download(self):
        ...
        with mock.patch.object(Downloader, 'source_matches_destination') as m:
            ...
            with mock.patch.object(Downloader, 'needs_downloading') as n:
                ...
                source = 'file:///b.txt'
                destination = '/a.txt'
                # Here a copy of b has been made in above function,now the downloder.needs_downloading() will return 
                # false an no download will happen.
                with mock.patch('cities_light.downloader.urlopen') as uo_mock:
                    downloader.download(source, destination)
                    uo_mock.assert_not_called()
